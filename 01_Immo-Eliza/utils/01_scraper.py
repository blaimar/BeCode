...
class QuotesSpider(scrapy.Spider):
    ...
    start_urls = [f"https://www.immoweb.be/en/search/{TYPE}/for-sale/abc/{LOCALITY}?countries=BE&page=333&orderBy=relevance" for LOCALITY in range(1000,9999+1,10) for TYPE in ["apartment","house"]]
    
    def parse(self,response):
        """
        We gonna make all the requests in start_urls and see which one doesnt works
        """
        ...
        if "10000 properties" not in verification:
            ...
            for i in range(1,last_page+1):
                ...
                yield scrapy.Request(url, callback=self.parse_all_propreties)
    ...
    def parse_specific_url(self, response):
        ...
        if pattern:
            ...
            print(f"Error with {response.url} => {response.status}")
